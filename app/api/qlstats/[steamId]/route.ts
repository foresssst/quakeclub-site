...
import { getPlayerStats, calculateKD, calculateWinRate, getRankName } from "@/lib/qlstats"
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ steamId: string }> }) {
  try {
    ...
    console.log("Recent games from API:", JSON.stringify(playerStats.recent_games?.slice(0, 2), null, 2))
    ...
    const mappedRecentGames = recentGamesFiltered.slice(0, 10).map((game) => {
      ...
      for (const [timestampStr, change] of eloChangesMap.entries()) {
        const recordTime = parseInt(timestampStr)
